...
{
    auto start = std::chrono::high_resolution_clock::now(); // tracking time spent from here 
    ...
    // parallely loading the gray_img vector with pixel values
    ...
    #pragma omp parallel shared (png_image,gray_img,n) private(i) 
    {
        #pragma omp for schedule(static)
        for(i=0;i<n;i++)
        {
            gray_img[i] = *(png_image + i);
        }
    }
    ...
    #pragma omp parallel shared(sk_round,gray_img) private(i)
    {
        #pragma omp for schedule(static)
        for(i=0;i<n;i++)
        {
            *(normalized_image + i) = (uint8_t) sk_round[gray_img[i]];
        }
    }
    ...
    auto duration_0 = std::chrono::duration_cast<std::chrono::milliseconds>(stop_0 - start);
    auto duration_1 = std::chrono::duration_cast<std::chrono::milliseconds>(stop_1 - start);
